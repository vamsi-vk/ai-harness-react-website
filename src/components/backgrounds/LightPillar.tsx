...
/** React Bits Light Pillar — vertical pillar of light with glow. */
const LightPillar: FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice =
      isMobile || (navigator.hardwareConcurrency !== undefined && navigator.hardwareConcurrency <= 4);
    ...
    // Mount once for quality/WebGL; prop updates sync in effects below.
  ...
  if (!webGLSupported) {
    return <div className={`light-pillar-fallback ${className}`}>WebGL not supported</div>;
  }
