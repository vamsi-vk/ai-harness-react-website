type LogoItem = {
  name: string;
  image?: string;
};

const logos: LogoItem[] = [
  { name: "Podium", image: "/podium-logo.png" }, { name: "Clintrust.ai", image: "/clintrust-logo.png" },
];

export default function LogoCloud() {
  return (
    <div className="relative mx-auto max-w-6xl">
      <div className="pointer-events-none absolute inset-y-0 left-0 z-10 hidden w-24 bg-gradient-to-r from-white to-transparent sm:block" />
      <div className="pointer-events-none absolute inset-y-0 right-0 z-10 hidden w-24 bg-gradient-to-l from-white to-transparent sm:block" />
      <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 py-2 sm:gap-12">
        {logos.map((logo) => (
          <div
            key={logo.name}
            className="flex items-center gap-3 text-ink-400"
          >
            {logo.image ? (
              <span className="grid h-8 w-8 place-items-center overflow-hidden rounded-md border border-ink-200 bg-white">
                <img src={logo.image} alt={`${logo.name} logo`} className="h-full w-full object-cover" loading="lazy" />
              </span>
            ) : (
              <span className="grid h-7 w-7 place-items-center rounded-md bg-ink-100 text-ink-500">
                <svg viewBox="0 0 20 20" className="h-4 w-4" fill="currentColor">
                  <circle cx="10" cy="10" r="3.2" />
                  <circle cx="10" cy="10" r="7" fill="none" stroke="currentColor" strokeWidth="1.2" opacity="0.6" />
                </svg>
              </span>
            )}
            <span className="text-[15px] font-semibold tracking-tight text-ink-500">{logo.name}</span>
          </div>
        ))}
      </div>
    </div>
  );
}
